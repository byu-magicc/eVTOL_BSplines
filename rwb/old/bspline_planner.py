...
class Planner:
    def __init__(self):
        self.knots = np.array([0, 1, 2])
        self.control_points = np.array([[1, 0, 0],
                                        [0, 1, 0],
                                        [0, 0, 1]])
        self.degree = 0
        self.num_segments = 2  # number of segments in time window
        # spline class from scipy library
        self.spl = BSpline(t=self.knots, c=self.control_points, k=self.degree)

    def set_knots_uniform(self, t0, tf, degree, num_segments):
